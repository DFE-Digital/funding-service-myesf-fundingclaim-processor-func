...
{
    [TestClass]
    public class GetFundingClaimTimerFunctionTests
    {
        ...
        public void Run_WhenUseNewAPI_DoesNotThrowException()
        {
            ...
            mockFundingClaimService
                .Setup(service => service.GetFundingClaims())
            ...
            var mockLogger = new Mock<ILoggerAdapter<GetFundingClaimTimerFunction>>();

            var function = new GetFundingClaimTimerFunction(
            ...
            mockFundingClaimService.Verify(
                mcs => mcs.GetFundingClaims(),
                Times.Once);
            mockLogger.Verify(
                l => l.LogInformation(
                    $"GetFundingClaim timer trigger function started at: {date}."),
                Times.Once);
        }
